...
//  std::cout << "Execute runTest member function from BlasTest object " << __FILE__ << "\t" <<__LINE__ << std::endl;
  ...
  for (auto c_blas_src: c_blas_srcs_) {
    cmd_result = compileTest(flog_, getCJobScripts()[c_i], c_blas_srcs_[c_i], getCName(), getCFlags(), getCSrcName(c_i), getCExeName(c_i), getCLinkLibs());
  ...
  for (auto cpp_blas_src: cpp_blas_srcs_) {
    cmd_result = compileTest(flog_, getCppJobScripts()[c_i], cpp_blas_srcs_[c_i], getCppName(), getCppFlags(), getCppSrcName(c_i), getCppExeName(c_i), getCppLinkLibs());
  ...
  for (auto f_blas_src: f_blas_srcs_) {
    cmd_result = compileTest(flog_, getFJobScripts()[c_i], f_blas_srcs_[c_i], getFName(), getFFlags(), getFSrcName(c_i), getFExeName(c_i), getFLinkLibs());
